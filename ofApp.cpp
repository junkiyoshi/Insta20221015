...
void ofApp::update() {
	...
	for (int radius = 50; radius <= 300; radius += 30) {
		...
		auto rotation = glm::vec3(
			ofMap(ofNoise(noise_param.x, radius * 0.00065 - ofGetFrameNum() * 0.008), 0, 1, -360, 360),
			ofMap(ofNoise(noise_param.y, radius * 0.00065 - ofGetFrameNum() * 0.008), 0, 1, -360, 360),
			ofMap(ofNoise(noise_param.z, radius * 0.00065 - ofGetFrameNum() * 0.008), 0, 1, -360, 360));
